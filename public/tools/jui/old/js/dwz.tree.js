 ...
 (function($){
 	$.extend($.fn, {
		...
		_checkParent:function(){
			//console.log($(this).find("a").attr("tvalue"));
			/* s.wx2016/2/9.保存最后一级menu的id------------------e */
			menuThisId = '';
			var t_arr = $(this).find("a").attr("tvalue").split('.');
			if ( t_arr[2] ){
				menuThisId = t_arr[2];
				if ( menuIds[menuThisId] ){
					delete menuIds[menuThisId];
				}else{
					menuIds[menuThisId] = menuThisId; 
				}
				var t_arr = [];
				var t_counter = 0;
				for( var i in menuIds ){
					if ( menuIds[i] ){
						t_arr[t_counter] = menuIds[i];
						t_counter++;
					}
				}
				if ( $("input[name='authority']") ){
					$("input[name='authority']").val(t_arr.join(","));
				}
