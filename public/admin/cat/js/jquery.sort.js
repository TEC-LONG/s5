...
function selectP(p) {
	...
	//将添加顶级分类标记按钮设置为switch-off
	$('.make-switch').find('div').attr('class', 'switch-off switch-animate');

	//当前选中项的值
	nowCat.id = province_ids[p];
	nowCat.name = province[p];
	nowCat.level = 1;
	//获取当前选中一级分类的子分类
	if ( typeof(city[p])==='undefined' ){
		$.ajax({
			...
			dataType:'json',
			url:domain+'/index.php?p=admin&m=cat&a=getChild',
			async:false,
			success:function (re){
				if ( re.length==0 ){
					city[p] = [];
...
function selectC(p,c) {
	...
	//当前选中项的值
	nowCat.id = city_ids[p][c];
	nowCat.name = city[p][c];
	nowCat.level = 2;
	//console.log(p);
	//console.log(c);
	//console.log(typeof(district[p]));
	//console.log(typeof(district[p][c]));
	//return false;
	//获取当前选中二级分类的子分类
		...
		$.ajax({
			...
			success:function (re){
				if ( re.length==0 ){
					...
					district[p][c] = re['s_names'];
					if ( typeof(district_ids[p])==='undefined' ){
						district_ids[p] = [];
					}
					district_ids[p][c] = re['s_ids'];
				}
			}
		});
	}
...
function selectD(p,c,d) {
	...
	//当前选中项的值
	nowCat.id = district_ids[p][c][d];
	nowCat.name = district[p][c][d];
	nowCat.level = 3;
