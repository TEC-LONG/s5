...
class TBRecordController extends Controller {
    ...
    public function index(){ 
        ...
        $sql = 'select id, post_date, belong_db, ch_name, en_name, has_special_field from tb_record where 1 order by post_date desc limit ' . $limit;
    ...
    public function adh(){
        ...
        if(!empty($request['special_fields'])){
            ...
            $sql = 'insert into tb_special_field (tb_record__id, en_name, serialize, field_type, post_date) values ';
        ...
        //成功
        $re = AJAXre();
        $re->navTabId = $this->_datas['navTab'].'_ad';
        $re->message = '添加成功！';
        #返回结果
        echo json_encode($re); 
        exit;
