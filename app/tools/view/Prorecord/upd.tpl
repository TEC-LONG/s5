...
<body>
	<ul class="nav nav-tabs">
		<li class="nav-item">
			<a class="nav-link active">编辑工程信息</a>
		</li>
	</ul>
	<form class="needs-validation" action="{$url.updh.url}?id={$row.id}" method="post">
		<div class="form-row d-flex mt-3">
			...
			<div class="col-md-2">
				...
				<input type="text" class="form-control" id="tools_prorecord_add_title" name="title" placeholder="必填项" value="{$row.title}" required>
			</div>
			<div class="col-md-1">
				<label>所属工程</label>
				<select name="belong_pro" class="form-control">
					{foreach $belong_pro as $belong_pro_key=>$belong_pro_val}
					<option value="{$belong_pro_key}" {if $belong_pro_key==$row['belong_pro']}selected{/if}>{$belong_pro_val}</option>
					{/foreach}
				</select>
			</div>
			<div class="col-md-1">
