...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="{$url.group.url}" method="post" onreset="$(this).find('select.combox').comboxReset()">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="{$url.gAdUpd.url}" target="dialog" rel="{$url.gAdUpd.rel}" minable="false" width="750" height="160"><span>新增用户组</span></a></li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid_{$navTab}" rel="{$row.id}">
				...
				<td>{$row.tags}</td>
				<td>
					<a title="确实要删除？" target="ajaxTodo" href="{$url.gPost.url}?type=del&id={$row['id']}" class="btnDel">删除</a>
