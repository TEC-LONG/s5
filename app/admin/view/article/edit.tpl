...
<body>
	...
	<form class="needs-validation" action="{$url.updh.url}?id={$row.id}" method="post">
		<div class="form-row d-flex mt-3">
			...
			<div class="col-md-2">
				<label for="admin_article_cat_title">文章标题</label>
				<input type="text" class="form-control" id="admin_article_cat_title" name="title" placeholder="必填项" {if isset($row.title)}value="{$row.title}"{/if} required>
			</div>
			<div class="col-md-3">
				<label for="admin_article_cat_tags">文章标签</label>
				<input type="text" class="form-control" id="admin_article_cat_tags" name="tags" {if isset($row.tags)}value="{$row.tags}"{/if}>
			</div>
			<div class="col-md-1">
				<label for="admin_article_cat_lv1">所属分类</label>
				<select class="form-control" id="admin_article_cat_lv1" name="cat1" onchange="cascade_this(this, 'lv1');">
					<option value="0">请选择...</option>
					{foreach $cat_lv1 as $k=>$v}
			...
			<div class="col-md-1 mt-2">
				<label> </label>
				<select class="form-control admin_article_cat_lv2" name="cat2" onchange="cascade_this(this, 'lv2');" disabled>
					<option>请选择...</option>
				</select>
			</div>
			<div  class="col-md-1 mt-2">
				<label> </label>
				<select class="form-control admin_article_cat_lv3" name="cat3" disabled>
					<option>请选择...</option>
				</select>
			</div>
			<div class="col-md-1">
...
var cat2 = "{$row['crumbs_cat_ids'][1]}|{$row['crumbs_cat_names'][1]}";
var cat3 = "{$row['crumbs_cat_ids'][2]}|{$row['crumbs_cat_names'][2]}";

var url = '{L("/article/cate/child")}';
var cascade = new Cascade(url, 'admin_article_cat_lv2', 'admin_article_cat_lv3');

cascade_this($('select[name="cat1"]')[0], 'lv1', function(){

	$('select[name="cat2"]').val(cat2);

	cascade_this($('select[name="cat2"]')[0], 'lv2', function(){

		$('select[name="cat3"]').val(cat3);
	})
});
