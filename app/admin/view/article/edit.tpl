...
<body>
	...
	<form class="needs-validation" action="{$url.updh.url}?id={$row.id}" method="post">
		<div class="form-row d-flex mt-3">
			...
			<div class="col-md-2">
				<label for="tools_exp_add_title">文章标题</label>
				<input type="text" class="form-control" id="tools_exp_add_title" name="title" placeholder="必填项" {if isset($row.title)}value="{$row.title}"{/if} required>
			</div>
			<div class="col-md-3">
				<label for="tools_exp_add_tags">文章标签</label>
				<input type="text" class="form-control" id="tools_exp_add_tags" name="tags" {if isset($row.title)}value="{$row.tags}"{/if}>
			</div>
			<div class="col-md-1">
				<label for="tools_exp_add_cat">所属分类</label>
...
function Cascade(url, lv2_class, lv3_class){
	...
	this.showNext = function (pid) {
		...
		$.ajax({
			...
			success:function (re){
				...
				if (type=='lv1') {
					$(this.lv2_class).html(options);
					$(this.lv2_class).removeAttr('disabled');
					this.disableLv3();
				} else {
					$(this.lv3_class).html(options);
	...
	this.clickLv = function (pid, type) {
	...
	this.disableLv2 = function () {
		$(this.lv2_class).html('<option value="0">请选择...</option>');
		$(this.lv2_class).attr('disabled', true);
	}

	this.disableLv3 = function () {
		$(lv3_class).html('<option value="0">请选择...</option>');
		$(lv3_class).attr('disabled', true);
	}
}
