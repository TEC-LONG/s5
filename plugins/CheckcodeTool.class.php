...
define('ADMIN_FONTS_DIR', dirname(__FILE__).'/Checkcode/');
class CheckcodeTool{
	...
	//public function __construct($width=700, $height=250, $fontsize=20, $fontspace=15, $left=15, $bottom=10){
	public function __construct($arr=array('width'=>700, 'height'=>250, 'fontsize'=>20, 'fontspace'=>15, 'left'=>15, 'bottom'=>10)){
		$this->_canvas_width = $arr['width'];
		$this->_canvas_height = $arr['height'];
        $this->_fontsize = $arr['fontsize'];
        $this->_fontspace = $arr['fontspace'];
        $this->_left = $arr['left'];
        $this->_bottom = $arr['bottom'];
...
//		exit;
	...
	public function animation($canvas_width, $canvas_height, $string){  //animation:动画
		...
		for ($i=0; $i<16; ++$i){
			...
			/* 在画布上写字 */
			$font_space = $this->_fontspace;
//			if($i>0){  /* 屏蔽第一帧 */
				for($j=0; $j<strlen($string); ++$j){
					$font_color = imagecolorallocate($img, rand(10, 80), rand(81, 170), rand(171, 255));
					$font_file = ADMIN_FONTS_DIR . 'font_1.ttf';
	
					imagettftext($img, $this->_fontsize, rand(-15, 15), $this->_left+$font_space*$j, imagesy($img)-$this->_bottom, $font_color, $font_file, $string[$j]);
				}
//			}
